...
// On STM32H7 flash sectors are used when erasing
            ...
            flash_erase.VoltageRange    = FLASH_VOLTAGE_RANGE_3; // VDD = 2.7 to 3.6V
#endif
        ...
        &&  ( NULL != p_data ))
    {
        ...
        if ( 0U == ( addr % 32 ))
        {
            ...
            for ( uint32_t address_offset = 0; address_offset < size; address_offset += 32 )
            {
                ...
                if ( HAL_OK != HAL_FLASH_Program( FLASH_TYPEPROGRAM_FLASHWORD, addr + address_offset, (uint32_t)(p_data + address_offset)))
                {
                    status = eFLASH_ERROR;
                    break;
